...
def choice_dolar_price():
    ...
    window_price_dolar_label1 = ttk.Label(
        window_price_dolar, text=f'Cena z pliku wynosi: {a}', style='core.TLabel', font=('Helvetica', 12), justify=LEFT)
    window_price_dolar_label1.grid(row=0, column=0, columnspan=2, padx=5)
    window_price_dolar_button1 = ttk.Button(
        window_price_dolar, text='Wybierz', style='primary.TButton', command=lambda: przypisz(a, b, 0))
    window_price_dolar_button1.grid(row=0, column=3, pady=5)

    window_price_dolar_label2 = ttk.Label(
        window_price_dolar, text=f'Cena pobrana wynosi: {b}', style='core.TLabel', font=('Helvetica', 12), justify=LEFT)
    window_price_dolar_label2.grid(row=1, columnspan=2, column=0, padx=5)
    window_price_dolar_button2 = ttk.Button(
        window_price_dolar, text='Wybierz', style='primary.TButton', command=lambda: przypisz(a, b, 1))
    window_price_dolar_button2.grid(row=1, column=3, pady=5)

    window_price_dolar_label3 = ttk.Label(
        window_price_dolar, text='Wprowadz własną cenę dolara: ', style='core.TLabel', font=('Helvetica', 12), justify=LEFT)
    ...
    window_price_dolar_button3 = ttk.Button(
        window_price_dolar, text='Wybierz', style='primary.TButton', command=lambda: przypisz(a, b, 2))
...
right_treev_wyniki['columns'] = ('Cena zl', 'Cena $', 'Wartość zl',
                                 'Wartość $', 'Zysk/Strata zl', 'Zysk/Strata $', 'Zysk/Strata %')
...
right_treev_wyniki.column("Zysk/Strata zl", width=87,
...
def frame_wyniki():
    ...
    wyniki_opis = ttk.Label(frame_wyniki, text='Podsumowanie portfela',
                            style='core.TLabel', font=('Helvetica', 12))
    wyniki_opis.grid(row=0, column=0, columnspan=2, pady=5, padx=5)

    wyniki_o1 = ttk.Label(frame_wyniki, text='Ogólny Zysk/Strata zł wynosi: ',
                          style='core.TLabel', font=('Helvetica', 12))
    ...
    wyniki_o2 = ttk.Label(frame_wyniki, text='Ogólny Zysk/Strata $ wynosi: ',
                          style='core.TLabel', font=('Helvetica', 12))
    ...
    wyniki_o3 = ttk.Label(frame_wyniki, text='Ogólny Zysk/Strata % wynosi: ',
                          style='core.TLabel', font=('Helvetica', 12))
    ...
    wyniki_e5 = ttk.Label(frame_wyniki, text='Wartość portfela w zł wynosi: ',
                          style='core.TLabel', font=('Helvetica', 12))
    ...
    wyniki_o5 = ttk.Label(frame_wyniki, text='Wartość zaiwestowana wynosi: ',
                          style='core.TLabel', font=('Helvetica', 12))
...
def krypto_price():
    ...
    for a in lista_krypto:
        ...
        r_wyniki[j].insert(2, float(dane_portfel[j][3] *
                           r_wyniki[j][0]).__round__(2))  # Wartość zł
        r_wyniki[j].insert(3, float(dane_portfel[j][3] *
                           price).__round__(2))  # Wartość $
        # Zysk_Strata zł
        r_wyniki[j].insert(
            4, (r_wyniki[j][2]-float(dane_portfel[j][1])).__round__(2))
        r_wyniki[j].insert(
            5, (r_wyniki[j][3]-float(dane_portfel[j][2])).__round__(2))  # Zysk_Strata $
        ...
        if (float(dane_portfel[j][1]) < 0):
            ...
            procent_z_zl = ((r_wyniki[j][4]*100) /
                            dane_portfel[j][1]).__round__(2)
    ...
    def wartosci(krypto: list, wynik: list, numer_zadania: int):
        ...
        for a in krypto:
            ...
            def portf(numer_zadania: int, j: int):
                ...
                if numer_zadania == 3:
                    tmp = tmp+6
                wynik[j].insert(2, float(dane_portfel[tmp][3]
                                * wynik[j][0]).__round__(2))  # Wartość zł
                wynik[j].insert(3, float(dane_portfel[tmp][3]
                                * price).__round__(2))  # Wartość $
                # Zysk_Strata zł
                wynik[j].insert(
                ...
                if (float(dane_portfel[tmp][1]) < 0):
                    ...
                    procent_z_zl = ((wynik[j][4]*100) /
                                    dane_portfel[tmp][1]).__round__(2)
...
top_switch_label = ttk.Label(
    frame_switch, text=f'Włącz odświeżanie danych co {czas}s ', style='core.TLabel', font=('Helvetica', 12))
...
def wykresy():
    ...
    label_wykresy = ttk.Label(frame_wykresy2, text='Wykresy z ostatnich 24h', style='core.TLabel', font=(
        'Helvetica', 12), background='#2c3e50', border=5)
    ...
    data_file = open(
        zmienne_file['Sciezka_Dane_wykres'], 'r', newline='').readline()
    ...
    if data_wykres == data_file:
        ...
        for i in t_lista:
            filepath = str(zmienne_file['URL_wykres_data']).replace(
                '{i}', f'{i}')
...
# region wykresu
    ...
    for j in range(len(dane_portfel)):
        ...
        if len(lista_wybrana[1]) != 0:
            tendencja_24 = str(
                (((lista_wybrana[1][(len(lista_wybrana[1])-1)]/lista_wybrana[1][0])-1)*100).__round__(2))
