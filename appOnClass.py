...
# root.style.configure('.', font=('Roboto Condensed', 11))
...
root.style.configure("primary.Button", highlightbackground=0, buttonuprelief="")
root.style.configure("Treeview.Heading", font=("Helvetica", 12))
core = ttk.Frame(root)
core.grid()

top_area = AreaFrame(onFrame=core, row=0, column=0, columnspan=3)
middle_area = AreaFrame(onFrame=core, row=1, column=0, columnspan=3)
bottom1_area = AreaFrame(
    onFrame=core, row=2, column=0, sticky="w"
)  # , height=100, width=100)
bottom2_area = AreaFrame(onFrame=core, row=2, column=1)  # , height=100, width=100)
bottom3_area = AreaFrame(onFrame=core, row=2, column=2)  # , height=100, width=100)
...
def price_wallet(wallet: list):
    ...
    pre_url = '","'.join([f"{i[0]}USDT" for i in wallet if i[0] != "ARI10"])
    url = (
        requests.get(
    ...
    for i in range(len(wallet)):
        try:
            ...
            if wallet[i][0] == "ARI10":
                downloadPrice = readFile.file_list[0]["Cena_ARI10"]
            else:
                downloadPrice = 0

        pricePln = (downloadPrice * read_file["Stable_price"]["Dolar"][0]).__round__(3)
        valuePln = (float(wallet[i][3]) * pricePln).__round__(2)
        valueDollar = (float(wallet[i][3]) * downloadPrice).__round__(2)

        def count_percent():
            result = ((profitLost * 100) / abs(float(wallet[i][1]))).__round__(2)
            return f"{result} %"
...
def top_area_ingredients():
    ...
    top_area.text_display(
        f'Włącz odświeżanie danych co {readFile.file_list[0]["czas_refresh"]}s',
        row=0,
        column=2,
    )
    top_area.button_display("OFF", row=0, column=3, method=button_change_time)
