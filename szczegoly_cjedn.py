...
def szczegoly_zakupow():
    ...
    szczegoly_zakupow_treeview=ttk.Treeview(szczegoly_zakupow_window,style='primary.Treeview',show='headings',selectmode='browse')
    szczegoly_zakupow_treeview['columns']=("Data","Krypto","Kupno/Sprzedaż","Cena zl","Cena $","Ilość")
        ...
        for a in odczyt:
            ...
            if i>=1:
                if search(szczegoly_zakupow_filtr_data,data)==False:
                    szczegoly_zakupow_filtr_data.append(data)
                if search(szczegoly_zakupow_filtr_krypto,szczegoly_zakupow_dane[i][1])==False:
                    szczegoly_zakupow_filtr_krypto.append(szczegoly_zakupow_dane[i][1])
                if search(szczegoly_zakupow_filtr_k_s,szczegoly_zakupow_dane[i][2])==False:
                    szczegoly_zakupow_filtr_k_s.append(szczegoly_zakupow_dane[i][2])
        ...
        for i in range(len(szczegoly_treeview_cjednostkowa_dane)):
            ...
            for j in range(len(dane_portfel)):
                ...
                if dane_portfel[j][0]==szczegoly_treeview_cjednostkowa_dane[i][0] and dane_portfel[j][3]!=szczegoly_treeview_cjednostkowa_dane[i][3]:
                    # warunek 0 lub mniej
                    dane_portfel[j][1]=str((szczegoly_treeview_cjednostkowa_dane[i][4]).__round__(2)) #zł
    ...
    def filtr_szczegoly_zakupow(event):
        ...
        if wybrana_data!='Wszystko':
            ...
            if wybrana_krypto!='Wszystko' and len(tabela_wybrana)>0:
                ...
                for i in range(len(tabela_wybrana)):
                    data=datetime.strptime((tabela_wybrana[i][0])[6:],'%Y').strftime('%Y')
                    ...
                    if tabela_wybrana[i][1]==wybrana_krypto:
                        ...
                        if search(szczegoly_zakupow_filtr_krypto2,tabela_wybrana[i][1])==False:
                            szczegoly_zakupow_filtr_krypto2.append(tabela_wybrana[i][1])
                        elif search(szczegoly_zakupow_filtr_k_s2,tabela_wybrana[i][2])==False:
                ...
                szczegoly_zakupow_combobox_krypto['values']=szczegoly_zakupow_filtr_krypto2
                ...
                for i in range(len(szczegoly_zakupow_dane)):
                    ...
                    if((szczegoly_zakupow_dane[i][0])[6:])==wybrana_data:
                        ...
                        if search(szczegoly_zakupow_filtr_data2,data)==False:
                            szczegoly_zakupow_filtr_data2.append(data)
                        elif search(szczegoly_zakupow_filtr_krypto2,szczegoly_zakupow_dane[i][1])==False:
                            szczegoly_zakupow_filtr_krypto2.append(szczegoly_zakupow_dane[i][1])
                        elif search(szczegoly_zakupow_filtr_k_s2,szczegoly_zakupow_dane[i][2])==False:
                            szczegoly_zakupow_filtr_k_s2.append(szczegoly_zakupow_dane[i][2])
        ...
        elif wybrana_data=='Wszystko' and wybrana_krypto=='Wszystko' and wybrana_b_s=='Wszystko':
    ...
    def edit_wybierz():
        selected_date=szczegoly_zakupow_treeview.item(szczegoly_zakupow_treeview.selection())['values'] #['02.04.2021', 'ETH', 'Sprzedaz', '15785.59', '4047.59', 2]
    ...
    szczegoly_zakupow_label_data=ttk.Label(szczegoly_zakupow_frame_filtry,text='Data:',style='core.TLabel',font=('Helvetica',12))
    szczegoly_zakupow_label_data.grid(row=0,column=0)
    szczegoly_zakupow_combobox_data=ttk.Combobox(szczegoly_zakupow_frame_filtry,text='Data')# Ustawić opcje bez edycji
    szczegoly_zakupow_combobox_data.configure(state='readonly',values=szczegoly_zakupow_filtr_data) #działa
    szczegoly_zakupow_combobox_data.current(0)
    szczegoly_zakupow_combobox_data.bind("<<ComboboxSelected>>",filtr_szczegoly_zakupow)# on choice
    szczegoly_zakupow_combobox_data.grid(row=1,column=0)

    szczegoly_zakupow_label_krypto=ttk.Label(szczegoly_zakupow_frame_filtry,text='Krypto:',style='core.TLabel',font=('Helvetica',12))
    szczegoly_zakupow_label_krypto.grid(row=0,column=1)
    szczegoly_zakupow_combobox_krypto=ttk.Combobox(szczegoly_zakupow_frame_filtry,text='krypto')
    szczegoly_zakupow_combobox_krypto.configure(state='readonly',values=szczegoly_zakupow_filtr_krypto)
    szczegoly_zakupow_combobox_krypto.current(0)
    szczegoly_zakupow_combobox_krypto.bind("<<ComboboxSelected>>",filtr_szczegoly_zakupow)
    szczegoly_zakupow_combobox_krypto.grid(row=1,column=1)
    
    szczegoly_zakupow_label_b_s=ttk.Label(szczegoly_zakupow_frame_filtry,text='Kupno/Sprzedaż:',style='core.TLabel',font=('Helvetica',12))
    szczegoly_zakupow_label_b_s.grid(row=0,column=2)
    szczegoly_zakupow_combobox_b_s=ttk.Combobox(szczegoly_zakupow_frame_filtry,text='kupno/Sprzedaż')
    szczegoly_zakupow_combobox_b_s.configure(state='readonly',values=szczegoly_zakupow_filtr_k_s)
    ...
    szczegoly_zakupow_entry_data=ttk.Entry(szczegoly_zakupow_frame_edit,style='primary.TEntry',width=11)
    szczegoly_zakupow_entry_data.grid(row=0,column=0)

    szczegoly_zakupow_entry_krypto=ttk.Entry(szczegoly_zakupow_frame_edit,style='primary.TEntry',width=11)
    szczegoly_zakupow_entry_krypto.grid(row=0,column=1)

    szczegoly_zakupow_entry_k_s=ttk.Entry(szczegoly_zakupow_frame_edit,style='primary.TEntry',width=11)
    szczegoly_zakupow_entry_k_s.grid(row=0,column=2)

    szczegoly_zakupow_entry_cena_zl=ttk.Entry(szczegoly_zakupow_frame_edit,style='primary.TEntry',width=11)
    szczegoly_zakupow_entry_cena_zl.grid(row=0,column=3)

    szczegoly_zakupow_entry_cena_d=ttk.Entry(szczegoly_zakupow_frame_edit,style='primary.TEntry',width=11)
    szczegoly_zakupow_entry_cena_d.grid(row=0,column=4)

    szczegoly_zakupow_entry_ilosc=ttk.Entry(szczegoly_zakupow_frame_edit,style='primary.TEntry',width=11)
    szczegoly_zakupow_entry_ilosc.grid(row=0,column=5)


    szczegoly_zakupow_frame_edit_button=Frame(szczegoly_zakupow_frame_edit,bg='#009999')
    szczegoly_zakupow_frame_edit_button.grid(row=1,column=0,columnspan=6)

    szczegoly_zakupow_button_wyczysc=ttk.Button(szczegoly_zakupow_frame_edit_button,text='Wyczyść',style='primary.TButton',command=clear_entry)
    szczegoly_zakupow_button_wyczysc.grid(row=0,column=0,padx=5,pady=5)

    szczegoly_zakupow_button_wybierz=ttk.Button(szczegoly_zakupow_frame_edit_button,text='Wybierz',style='primary.TButton',command=edit_wybierz)
    szczegoly_zakupow_button_wybierz.grid(row=0,column=1,padx=5,pady=5)

    szczegoly_zakupow_button_zamien=ttk.Button(szczegoly_zakupow_frame_edit_button,text='Zamień',style='primary.TButton',command=edit_zamien)
    szczegoly_zakupow_button_zamien.grid(row=0,column=2,padx=5,pady=5)

    szczegoly_zakupow_button_dodaj=ttk.Button(szczegoly_zakupow_frame_edit_button,text='Dodaj',style='primary.TButton',command=edit_dodaj)
    szczegoly_zakupow_button_dodaj.grid(row=0,column=3,padx=5,pady=5)

    szczegoly_zakupow_button_usun=ttk.Button(szczegoly_zakupow_frame_edit_button,text='Usuń',style='primary.TButton',command=edit_usun)
    szczegoly_zakupow_button_usun.grid(row=0,column=4,padx=5,pady=5)

    szczegoly_zakupow_button_update_portfel=ttk.Button(szczegoly_zakupow_frame_edit_button,text='Aktualizacja\n Portfela',command=update_portfel)
